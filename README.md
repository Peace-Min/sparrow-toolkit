...
> **진입점은 어느 바이너리를 띄웠는지 밝힌다.** 발행본(`publish\`)이 있으면 그것을 우선 실행하면서
> `[INFO] Launching published bundle: <경로>` 를 찍고, 없으면 `dotnet run` 폴백임을 `[INFO]` 로 알린다.
> 그리고 **발행본이 로컬 Release 빌드보다 오래됐으면 `[WARN]` 로 경고**한다 — 낡은 발행본이 조용히
> 우선 실행돼 "고친 게 반영이 안 된다"로 보이는 사고가 실제로 있었다.
> **소스를 고쳤는데 화면이 그대로면 이 `[INFO]`/`[WARN]` 줄부터 확인한다** →
> [docs/usage.md](docs/usage.md#진입점이-알려-주는-실행-바이너리와-낡은-발행본-경고).
...
|---|---|---|---|---|
| **코드 자동수정 (C#)** | **[코드 규칙]** / **[주석·레이아웃]** | **대상 폴더** 하나 | **로컬 소스 스캔(폴더 구조 그대로)** | 소스 파일을 **수정**(파괴적) · **C# 전용** |
...
- **[코드 자동수정]의 대상 선택은 폴더 하나다.** 버튼은 **[폴더 선택]** 과 **[새로고침]** 둘뿐이다
  (예전의 `[파일 선택]` 은 제거했다 — 어떤 파일을 골라도 부모 폴더로 환원돼 [폴더 선택]과 결과가 같았고,
  "sln 을 골라야 하나?"라는 오해만 만들었다). **경로를 직접 입력·붙여넣는 길은 그대로**라
  `.sln`/`.csproj` 문자열도 계속 받아 부모 폴더로 환원한다. 조작 상세는
  [docs/usage.md](docs/usage.md#대상-고르기).
- **[코드 자동수정]의 범위 트리는 `.sln` 을 파싱하지 않는다.** `.sln`/`.csproj` 를 주면 **부모 폴더**를,
  폴더를 주면 그 폴더를 루트로 잡고 폴더 구조 그대로 보여 준다 — 러너와 **같은 규칙**이라 트리에 보이는 것과
  실제로 고쳐지는 것이 일치한다(sln 에 없는 프로젝트·루트 레벨 `.cs` 가 빠지지 않는다).
- **git 이 없는 폴더**를 대상으로 잡으면 되돌릴 수단이 없으므로 **[규칙별 커밋 생성]만 잠그고** 안내와
  **[git 저장소 만들기]** 버튼을 띄운다. **실행 자체는 막지 않는다**(SVN 등 다른 버전관리 사용처를 위해).
  CLI 러너도 같다 — `-Commit` 인데 git 저장소가 아니면 규칙 실행 **전에** 사유를 알리고 커밋만 건너뛴다.
  판정 규칙과 화면 동작은 [docs/usage.md](docs/usage.md#git-이-없는-폴더를-대상으로-잡으면) 가 정본이다.
