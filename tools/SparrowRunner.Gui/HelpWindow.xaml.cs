    ...
    public partial class HelpWindow : Window
    {
        ...
        private void ShowTopic(string topic)
        {
            (string title, string[] paragraphs) = topic switch
            {
                ...
                "code-rules" => ("코드 규칙", new[] { "설정 > 코드 규칙에서 C/C++ 또는 C# 탭의 적용 항목을 선택한 뒤 실행 > 실행 > 코드 규칙 수정 실행(F7)을 선택합니다. 각 언어 탭에서 선택한 규칙은 해당 언어의 소스 파일에만 적용됩니다.", "프로젝트 탐색기에서 파일을 한 번 클릭하면 미리보고, 두 번 클릭하면 여러 파일을 탭으로 고정할 수 있습니다." }),
                ...
                _ => ("Sparrow Helper 시작하기", new[] { "Sparrow Helper는 C/C++ 및 C# 탭에서 선택한 규칙을 소스에 적용하고, Sparrow 분석 결과 XLS를 체커별 Markdown 파일로 분리하는 도구입니다.", "코드 자동수정 또는 XLS 분리 (모든 언어) 작업을 선택하세요. 창 어디에서든 F1 키를 누르면 도움말을 열 수 있습니다." })
            };
