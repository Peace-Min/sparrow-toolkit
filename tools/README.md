...
- `Run-SparrowRunnerGui.cmd`: **[코드 자동수정 (C#)]** ([코드 규칙] · [주석·레이아웃])과 **[XLS 분리 (모든 언어)]** 를 한 화면에서 실행하는 권장 진입점입니다. [XLS 분리]는 Sparrow 결과 XLS 하나만 받아 검출 전건을 체커 키 폴더별 항목 md(`<체커키>\{ID}_{파일명}_{라인}.md`)로 분리합니다. 인덱스/요약 파일은 만들지 않으며, 준비해야 할 선행 파일도 없습니다.
  ...
  - **어느 바이너리가 실행되는지 콘솔 첫 줄에 찍힙니다.** 발행본이 있으면 `[INFO] Launching published bundle: <경로>` 를 출력하고 그것을 우선 실행하며, 없으면 `dotnet run` 폴백임을 `[INFO]` 로 알립니다. 발행본이 로컬 Release 빌드보다 오래됐으면 `[WARN]` 5줄로 경고하고 재발행/삭제 방법을 안내합니다. **"고친 게 반영이 안 된다" 싶으면 이 줄부터 확인**하세요 → [`docs/usage.md`](../docs/usage.md#진입점이-알려-주는-실행-바이너리와-낡은-발행본-경고).
  - **대상 선택은 폴더 하나입니다.** 화면 맨 위 `대상 폴더` 입력 옆에 **[폴더 선택]** 과 **[새로고침]** 두 버튼만 있습니다(`.sln`/`.csproj` 경로를 직접 붙여넣으면 그 부모 폴더를 대상으로 씁니다).
- `Run-SparrowAll.cmd`: GUI 없이 [코드 규칙]·[주석·레이아웃]만 순차 실행해야 할 때 쓰는 보조 진입점입니다.
...
- `_internal/SparrowSyntaxFix`: [코드 규칙] 자동수정 엔진.
- `_internal/SparrowCommentFix`: [주석·레이아웃] 자동수정 엔진.
- `_internal/SparrowXlsExport`: [XLS 분리] XLS 파서 CLI와 테스트용 도구입니다.
- `_internal/SparrowXlsExport.Core`: [XLS 분리] 체커별 md 분리 익스포터 공용 라이브러리.
...
2. **레포 폴더 트리 전체**(발행된 `publish\` 산출물 포함)를 폐쇄망 PC로 복사합니다. [XLS 분리] 익스포터는 선행 문서를 읽지 않으므로 따로 챙길 자료는 없습니다.
...
- 규칙 추가 / 새 언어 갈래 추가: [`docs/extending.md`](../docs/extending.md)
