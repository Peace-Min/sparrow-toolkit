...
<#
    원큐 러너 — 코드 규칙(SparrowSyntaxFix, Roslyn) + 주석·레이아웃(SparrowCommentFix)을 한 번의 명령으로 순차 실행.
    ...
    기본 규칙셋은 이 스킬이 결정론으로 처리 가능한 [코드 규칙]·[주석·레이아웃] 전 규칙(신규 opt-in forvar/fieldsplit/emptystmt/
    forhoist/blockpromote 포함)이다. 검토필요(review-needed) 규칙은 커밋 메시지에 '검토필요' 가 드러난다 —
    [코드 규칙] 는 접두까지 갈라져 'sparrow(rule)! 검토필요: ...' 이고(무엇이 review-needed 인지는 SparrowSyntaxFix\README.md
    규칙 표가 단일 진실), [주석·레이아웃] 는 접두가 항상 'sparrow(comment): ' 이라 '!' 없이 라벨 본문에만 '검토필요' 가 붙는다
    (blockpromote 하나뿐). 즉 '검토필요 커밋만 골라내기'는 '!' 가 아니라 '검토필요' 로 grep 해야 둘 다 잡힌다.
    (참고: 예전 dotnet-format 기반 러너는 MyApp x64(레거시 비-SDK 프로젝트)에서 무력이라 삭제했고, 지금은 Roslyn 기반 SparrowSyntaxFix 러너를 쓴다.)

    사용:
      .\Run-SparrowAll.ps1 -Solution ...\MyApp.sln -Commit     # 코드 규칙→주석·레이아웃 전 규칙, 규칙별 자동 커밋
      .\Run-SparrowAll.ps1 -Solution ...\MyApp.sln -NoCommit   # 코드 규칙→주석·레이아웃 전 규칙, 파일만 수정
...
param(
    ...
    [switch]$NoCommit,
    [switch]$DryRun,
    # [코드 규칙] 엔진(SparrowSyntaxFix) 규칙 — 안전 기본(obviousvar/objectvar-safe/parens/foreachcast) + opt-in(forvar/fieldsplit/emptystmt/forhoist)
    [string]$SyntaxRules = 'obviousvar,objectvar-safe,parens,foreachcast,forvar,fieldsplit,emptystmt,forhoist',
    # [주석·레이아웃] 엔진(SparrowCommentFix) 규칙 — 전 규칙(continuation 깊이정규화·blockpromote opt-in 포함)
...
# 첫 갈래가 실패해도 두 번째 갈래는 계속 실행하고(둘 다 보고), 마지막에 하나라도 실패면 nonzero로 종료.
$failedModes = @()
$firstFailExit = 0

Write-Host "==================== 원큐: 코드 규칙 (SparrowSyntaxFix) ===================="
Write-Host "  규칙: $($syntaxList -join ',')"
& $syntaxRunner -Solution $Solution -Rules $syntaxList @pass
$codeRuleExit = $LASTEXITCODE
if ($null -eq $codeRuleExit) { $codeRuleExit = 0 }
Write-Host "  [코드 규칙 종료코드] $codeRuleExit"
if ($codeRuleExit -ne 0) {
    $failedModes += "코드 규칙(SparrowSyntaxFix) exit=$codeRuleExit"
    if ($firstFailExit -eq 0) { $firstFailExit = $codeRuleExit }
}

Write-Host ""
Write-Host "==================== 원큐: 주석·레이아웃 (SparrowCommentFix) ===================="
Write-Host "  규칙: $($commentList -join ',')"
& $commentRunner -Solution $Solution -Rules $commentList @pass
$commentExit = $LASTEXITCODE
if ($null -eq $commentExit) { $commentExit = 0 }
Write-Host "  [주석·레이아웃 종료코드] $commentExit"
if ($commentExit -ne 0) {
    $failedModes += "주석·레이아웃(SparrowCommentFix) exit=$commentExit"
    if ($firstFailExit -eq 0) { $firstFailExit = $commentExit }
}

Write-Host ""
if ($failedModes.Count -gt 0) {
    Write-Host "==================== 원큐 실패 ====================" -ForegroundColor Red
    Write-Host "실패 갈래: $($failedModes -join ' / ')" -ForegroundColor Red
    Write-Host "해당 갈래의 로그(Run-Sparrow*.log)를 확인하세요. (원큐 종료코드 $firstFailExit)"
    exit $firstFailExit
}

Write-Host "원큐 완료 (코드 규칙→주석·레이아웃). 다음(필수): (1) 빌드 통과 확인  (2) Sparrow 재분석으로 검출 감소 실측."
Write-Host "참고: 로직 수정 작업 후에는 이 원큐를 한 번 더 돌려 새로 생긴 형식 검출을 정리하세요."
