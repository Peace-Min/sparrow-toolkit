...
<#
    ...
    automatically with the detected checkers loaded. It then verifies, in order:
      ...
      G) 로컬 소스 범위 트리 = 폴더 구조 · git 게이트: a synthetic layout whose T.sln declares ONLY ProjA while
         .cs files also live OUTSIDE it (다른 프로젝트 · 루트 레벨 · 느슨한 폴더). Asserted: the tree's .cs set
         EQUALS the runner/engine's target set (FileDiscovery 규칙을 그대로 옮겨 계산해 대조), the tree root is the
         ROOT FOLDER name (not the sln name / project grouping), and 생성 파일·obj 아래 파일은 여전히 제외된다.
         Then, on that non-git folder: [규칙별 커밋 생성] is DISABLED + unchecked, the 안내 문구가 화면에 뜨고,
         [git 저장소 만들기](InitGitButton) 로 저장소를 만들면 체크박스가 다시 활성화된다 — 그 사이에도
         실행 버튼은 계속 활성이다(사내 SVN 사용처가 있어 git 없다고 차단하면 안 된다).
...
# ---- (G) 로컬 소스 범위 트리 픽스처 · git 게이트 ------------------------------
# sln 은 ProjA 만 선언하지만 프로젝트 밖에도 .cs 가 있다. 예전 GUI 는 sln 이 선언한 프로젝트 폴더 아래만
# 트리에 담아 나머지를 영원히 못 고쳤다(커버리지 구멍 — GUI 는 체크된 파일만 --files-from 으로 넘긴다).
# 이제 트리는 러너와 같은 규칙(파일이면 부모 폴더 = 루트)으로 만들어지므로 대상 집합이 같아야 한다.
$FIXSRC_DIR      = 'src'                      # $work 아래 소스 루트 폴더명 = 트리 루트 노드 이름
# 러너(=엔진)와 GUI 가 똑같이 대상으로 삼아야 하는 .cs (파일명이 서로 겹치지 않게 지어 이름으로 대조 가능).
$FIXSRC_EXPECT   = @('InProject.cs', 'NotInSln.cs', 'RootLevel.cs', 'Stray.cs')
# 양쪽 모두 제외해야 하는 것: 생성 파일 이름 규칙(.Designer.cs) · 자동생성 헤더가 붙은 obj 아래 파일.
$FIXSRC_EXCLUDED = @('Legacy.Designer.cs', 'Temp.cs')
...
# ---- (G) 합성 소스 배치 + 러너 대상 집합 계산 -----------------------------
# sln 이 일부만 선언한 배치를 만든다. 반환값 = 소스 루트($work\src) 전체 경로.
#   src\T.sln                     ← ProjA 만 선언
#   src\ProjA\InProject.cs        ← sln 이 아는 프로젝트 안
#   src\ProjB_외부\NotInSln.cs    ← sln 에 없는 프로젝트
#   src\RootLevel.cs              ← 루트 레벨(어느 프로젝트에도 안 속함)
#   src\느슨한폴더\Stray.cs       ← 프로젝트 아닌 폴더
#   src\ProjA\Legacy.Designer.cs  ← 생성 파일(이름 규칙) → 양쪽 제외
#   src\ProjA\obj\Debug\Temp.cs   ← obj 아래 + 자동생성 헤더 → 양쪽 제외
function New-FixSourceTree([string]$workDir) {
    $root = Join-Path $workDir $FIXSRC_DIR
    foreach ($d in @('ProjA', 'ProjB_외부', '느슨한폴더', 'ProjA\obj\Debug')) {
...
'@
    # 자동생성 헤더는 엔진(HasAutoGeneratedHeader)과 GUI 가 똑같이 보는 표식이다.
    $genHeader = "// <auto-" + "generated/>`r`n"
    $files = @(
        @{ Rel = 'ProjA\InProject.cs';       Name = 'InProject'; Prefix = '' },
        @{ Rel = 'ProjB_외부\NotInSln.cs';   Name = 'NotInSln';  Prefix = '' },
        @{ Rel = 'RootLevel.cs';             Name = 'RootLevel'; Prefix = '' },
        @{ Rel = '느슨한폴더\Stray.cs';      Name = 'Stray';     Prefix = '' },
        @{ Rel = 'ProjA\Legacy.Designer.cs'; Name = 'Legacy';    Prefix = '' },
        @{ Rel = 'ProjA\obj\Debug\Temp.cs';  Name = 'Temp';      Prefix = $genHeader }
    )
    foreach ($f in $files) {
        $text = [string]$f.Prefix + ($tpl -replace '@NAME@', [string]$f.Name)
        [System.IO.File]::WriteAllText((Join-Path $root ([string]$f.Rel)), $text, $u8)
    }
...
EndGlobal
'@
    [System.IO.File]::WriteAllText((Join-Path $root 'T.sln'), $sln, $u8)
    [System.IO.File]::WriteAllText((Join-Path $root 'ProjA\ProjA.csproj'),
        '<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net8.0</TargetFramework></PropertyGroup></Project>', $u8)
    return $root
}

# 러너/엔진이 실제로 대상 삼는 .cs 파일명 집합. SparrowSyntaxFix 의 FileDiscovery(IsGeneratedName +
# HasAutoGeneratedHeader)를 그대로 옮긴 것 — GUI 트리가 이 집합과 같아야 "커버리지 구멍 없음"이 증명된다.
function Get-RunnerTargetCsNames([string]$Root) {
    $suffixes = @('.Designer.cs', '.g.cs', '.g.i.cs', '.AssemblyInfo.cs', '.generated.cs')
    $out = New-Object System.Collections.Generic.List[string]
    foreach ($f in @(Get-ChildItem -LiteralPath $Root -Recurse -File -Filter *.cs -ErrorAction SilentlyContinue)) {
        $n = $f.Name
        if ($n -ieq 'AssemblyInfo.cs') { continue }
        if ($n -like 'TemporaryGeneratedFile_*') { continue }
        $skip = $false
        foreach ($s in $suffixes) { if ($n.EndsWith($s, [System.StringComparison]::OrdinalIgnoreCase)) { $skip = $true; break } }
        if ($skip) { continue }
        $head = (@(Get-Content -LiteralPath $f.FullName -TotalCount 3 -ErrorAction SilentlyContinue) -join "`n")
        if ($head -match '<auto-generated') { continue }
        [void]$out.Add($n)
    }
    return @($out | Sort-Object -Unique)
}

# git 저장소 안인가(위로 올라가며 .git 탐색 = git 자신의 판정). %TEMP% 가 이미 저장소 안인 드문 환경에서
# git 게이트 단정을 건너뛰기 위한 사전 확인.
function Test-InsideGitRepo([string]$path) {
...
function Invoke-OneIteration([int]$iter) {
    ...
    try {
        ...
        # ============ (G) 로컬 소스 범위 트리 = 폴더 구조(sln 파싱 없음) ============
        # 합성 배치: T.sln 은 ProjA 만 선언하는데 프로젝트 밖에도 .cs 가 있다. 트리가 sln 을 파싱하면
        # 그 파일들이 안 보이고, GUI 는 체크된 파일만 --files-from 으로 넘기므로 영원히 안 고쳐진다.
        # 여기서는 "트리에 보이는 .cs 집합 == 러너/엔진이 대상 삼는 .cs 집합" 을 직접 대조한다.
        $srcRoot = New-FixSourceTree $work
        $runnerTargets = @(Get-RunnerTargetCsNames $srcRoot)
        Check "G) [사전] 러너 대상 집합 = 기대 $($FIXSRC_EXPECT.Count)개 (실제: $($runnerTargets -join ' | '))" {
            ($runnerTargets.Count -eq $FIXSRC_EXPECT.Count) -and
            (@($FIXSRC_EXPECT | Where-Object { $runnerTargets -contains $_ }).Count -eq $FIXSRC_EXPECT.Count)
        }

        # .sln 을 대상으로 준다 — 러너와 같은 규칙이면 '부모 폴더'가 루트가 되어야 한다.
        UIA-SetValue (UIA-First $main 'TargetPathBox') (Join-Path $srcRoot 'T.sln')
        $scopeFilled = Wait-For {
            $t = UIA-First $main 'ScopeTree'
            if (-not $t) { return $false }
            Expand-AllTreeItems $t 4
            $names = @(Get-TreeItemNames $t)
            (@($FIXSRC_EXPECT | Where-Object { $names -contains $_ }).Count -eq $FIXSRC_EXPECT.Count)
        } 25
        $scopeTree = UIA-First $main 'ScopeTree'
        Expand-AllTreeItems $scopeTree 4
        $treeNames = @(Get-TreeItemNames $scopeTree)
        $treeCs = @($treeNames | Where-Object { $_ -like '*.cs' } | Sort-Object -Unique)
        Check "G) [핵심] 트리의 .cs 집합 == 러너 대상 집합 (트리: $($treeCs -join ' | ') / 러너: $($runnerTargets -join ' | '))" {
            $scopeFilled -and ($treeCs.Count -eq $runnerTargets.Count) -and
            (@($runnerTargets | Where-Object { $treeCs -contains $_ }).Count -eq $runnerTargets.Count)
        }
        Check "G) [커버리지] sln 밖 .cs 가 전부 트리에 보임(NotInSln.cs · RootLevel.cs · Stray.cs)" {
            ($treeCs -contains 'NotInSln.cs') -and ($treeCs -contains 'RootLevel.cs') -and ($treeCs -contains 'Stray.cs')
        }
        Check "G) [제외 유지] 생성 파일·obj 아래 파일은 트리에 없음($($FIXSRC_EXCLUDED -join ' · '))" {
            (@($FIXSRC_EXCLUDED | Where-Object { $treeCs -contains $_ }).Count -eq 0)
        }
        $scopeRootNames = @((Get-TreeRootItems $scopeTree) | ForEach-Object { [string]$_.Current.Name })
        Check "G) 트리 루트 = 루트 폴더명 '$FIXSRC_DIR' 하나(sln 이름/프로젝트 그룹 아님) (실제: $($scopeRootNames -join ' | '))" {
            ($scopeRootNames.Count -eq 1) -and ($scopeRootNames[0] -eq $FIXSRC_DIR)
        }
        $scopeStatus = UIA-FirstName $main 'ScopeStatusText'
        Check "G) 범위 요약이 선택/발견/제외를 보고 (실제: '$scopeStatus')" {
            ($scopeStatus -like "*$($FIXSRC_EXPECT.Count)개 선택*") -and ($scopeStatus -like '*개 제외*')
        }

        # ============ (G) git 미구축 게이트 ============
        # git 아닌 폴더에서는 커밋 옵션만 잠그고 안내한다. 실행 버튼은 절대 막지 않는다(사내 SVN 사용처).
        $gitCmd = Get-Command git -ErrorAction SilentlyContinue
        $tempInRepo = Test-InsideGitRepo $srcRoot
        if ($tempInRepo) {
            Log "  [skip] git 게이트 단정: 임시 폴더가 이미 git 저장소 안입니다($srcRoot)"
        }
        else {
            $commitBox2 = UIA-First $main 'CommitCheck'
            Check "G) git 아닌 대상: [규칙별 커밋 생성] 비활성" {
                ($null -ne $commitBox2) -and (-not $commitBox2.Current.IsEnabled)
            }
            if ($commitBox2) {
                $tog2 = $commitBox2.GetCurrentPattern([System.Windows.Automation.TogglePattern]::Pattern)
                Check "G) git 아닌 대상: 커밋 체크 해제 상태 (실제: $($tog2.Current.ToggleState))" {
                    $tog2.Current.ToggleState -eq 'Off'
                }
            }
            $gitTexts = @(Get-AllVisibleText $main)
            $noticeHits = @(Find-TextMatches $gitTexts 'git 저장소가 아닙니다')
            Check "G) 화면에 git 미구축 안내 표시(되돌릴 수단 없음 + 조치) (적중 $($noticeHits.Count)개)" {
                $noticeHits.Count -ge 1
            }
            $initBtn = UIA-First $main 'InitGitButton'
            Check "G) [git 저장소 만들기] 버튼(InitGitButton) 존재" { $null -ne $initBtn }
            $runBtn2 = UIA-First $main 'RunButton'
            Check "G) [핵심] git 없어도 실행 버튼은 계속 활성(차단 금지)" {
                ($null -ne $runBtn2) -and $runBtn2.Current.IsEnabled
            }

            if (-not $gitCmd) {
                Log "  [skip] git 저장소 생성 단정: git 이 PATH 에 없습니다"
            }
            elseif ($initBtn) {
                UIA-Invoke $initBtn
                $confirmedGit = Click-ConfirmYes $main 'git 저장소 만들기' 10
                Check "G) [git 저장소 만들기] 확인 대화상자에 '예' 응답" { $confirmedGit }
                $gitMade = Wait-For { Test-Path -LiteralPath (Join-Path $srcRoot '.git') } 30
                Check "G) git init 수행 → .git 생성" { $gitMade }
                $commitReEnabled = Wait-For {
                    $c = UIA-First $main 'CommitCheck'
                    ($null -ne $c) -and $c.Current.IsEnabled
                } 25
                Check "G) 저장소 생성 후 커밋 체크박스 활성화(상태 재판정)" { $commitReEnabled }
                Check "G) 저장소 생성 후 git 안내/[git 저장소 만들기] 사라짐" {
                    $null -eq (UIA-First $main 'InitGitButton')
                }
                $runBtn3 = UIA-First $main 'RunButton'
                Check "G) 저장소 생성 후에도 실행 버튼 활성" { ($null -ne $runBtn3) -and $runBtn3.Current.IsEnabled }
            }
        }
        Write-UiaTree $main "iteration $iter · 8단계: 로컬 소스 범위 트리 + git 게이트" ("tree-8-scope-git-iter$iter.txt") | Out-Null
...
# ---- (G) 소스 계약: 범위 트리는 sln 을 파싱하지 않는다 · 러너는 커밋 전에 git 을 판정한다 ----
# UIA 단정이 "결과"를 본다면 이 블록은 "구현이 되돌아가지 않았는지"를 본다(sln 파싱 부활 방지).
$scopeSource = [System.IO.File]::ReadAllText((Join-Path $RepositoryRoot 'tools\SparrowRunner.Gui\SourceScopeDiscovery.cs'))
Check "G) [핵심] 범위 탐색에 sln 파싱 경로 없음(DiscoverSolution/ReadSolutionProjects 제거)" {
    (-not $scopeSource.Contains('DiscoverSolution')) -and (-not $scopeSource.Contains('ReadSolutionProjects'))
}
Check "G) 범위 제외 규칙 보존(bin/obj/.git/.vs/packages · 생성 파일)" {
    $scopeSource.Contains('".git", ".vs", "bin", "obj", "packages"') -and
    $scopeSource.Contains('IsGeneratedOrBackup') -and $scopeSource.Contains('HasAutoGeneratedHeader')
}
Check "G) GUI 가 상위 폴더까지 올라가며 git 을 판정(FindGitRepositoryRoot)" {
    $guiSource.Contains('FindGitRepositoryRoot')
}
Check "G) GUI 는 git 미구축 시 커밋 옵션만 잠근다(_commitBlockedByGit 은 실행 버튼 판정에 안 쓰임)" {
    $guiSource.Contains('_commitBlockedByGit') -and
    $guiSource.Contains('CommitCheck.IsEnabled = commitApplies && _cts == null && !_commitBlockedByGit;') -and
    (-not $guiSource.Contains('RunButton.IsEnabled = _cts == null && !_commitBlockedByGit'))
}
foreach ($runnerRel in @('tools\_internal\SparrowSyntaxFix\Run-SparrowSyntaxFix.ps1',
                         'tools\_internal\SparrowCommentFix\Run-SparrowCommentFix.ps1')) {
    $runnerSrc = [System.IO.File]::ReadAllText((Join-Path $RepositoryRoot $runnerRel))
    Check "G) 러너가 -Commit 전에 git 저장소 여부를 판정: $(Split-Path $runnerRel -Leaf)" {
        $runnerSrc.Contains('function Test-GitRepository') -and $runnerSrc.Contains('$commitSkipReason')
    }
    Check "G) 러너가 커밋 생략 사유를 정확히 말함(락 재시도 오진 아님): $(Split-Path $runnerRel -Leaf)" {
        $runnerSrc.Contains('대상 루트가 git 저장소가 아닙니다')
    }
}
