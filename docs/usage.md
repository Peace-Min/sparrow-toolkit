...
CLI 러너도 같은 판정을 한다: `-Commit` 인데 대상 루트가 git 저장소가 아니면 **규칙을 돌리기 전에** 사유를 알리고 커밋 단계를 건너뛴다(파일 수정은 그대로 진행). 예전처럼 git 사용법 도움말이 콘솔에 쏟아지거나 "git 락 5회 재시도 후에도 실패"라는 **오진**이 나오지 않는다.
