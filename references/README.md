...
| 파일 | 성격 | 무엇 |
|---|---|---|
| [code-rule-autofix.md](code-rule-autofix.md) | **운영 정책 (현행)** | [코드 규칙] 자동수정의 운영 원칙, Sparrow 체커 ↔ [코드 규칙] 규칙 대응표, 실행/검증 절차 |
| [code-rule-roslyn-policy.md](code-rule-roslyn-policy.md) | **설계 기록 (2026-07 시점)** | [코드 규칙]을 Roslyn 규칙으로 확장하기로 한 설계안 — 규칙별 변환 계약·안전 조건·skip 조건·커밋명 규약·fixture 필수 케이스. **작성 시점 기준 문서라 "현재 2개 규칙" 같은 서술은 이미 낡았다**(현재 14개 규칙 키). 현행 규칙 목록은 [엔진 README](../tools/_internal/SparrowSyntaxFix/README.md) 를 본다 |
...
| 폴더 | 무엇 | 왜 커밋 안 하나 |
|---|---|---|
| `references/checkers/` | 체커별 조치 규칙 라이브러리(`<이름>.md`) + 지정 파일(`_assignments.json`) | 폐쇄망/사내 지식이다. [XLS 분리] 는 이게 없어도 완전히 동작한다(순수 익스포터) |
